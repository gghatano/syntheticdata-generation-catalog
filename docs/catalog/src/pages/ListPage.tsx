...
import { ExperimentCaseCard } from "../components/ExperimentCaseCard";
import { BeginnerGuide } from "../components/BeginnerGuide";
import type { DataCategory } from "../types/experiment-case";
import { DATA_CATEGORY_LABELS, DATA_CATEGORY_ICONS } from "../types/experiment-case";

type ViewTab = "cases" | "algorithms";
...
export function ListPage() {
  ...
  const { algorithms, loading: algoLoading, error: algoError } = useAlgorithms();
  const { cases, loading: casesLoading, error: casesError } = useExperimentCases();
  ...
  const loading = algoLoading || casesLoading;
  const error = algoError || casesError;
  ...
  const filteredCases = selectedCategory
    ? cases.filter((c) => c.data_category === selectedCategory)
    : cases;
      ...
      {activeTab === "cases" && (
        <div>
          <p className="text-sm text-gray-600 mb-4">
            実際のデータセットで合成データ生成を試した結果を、シナリオ別にまとめています。
            あなたのデータに近い事例を参考にしてください。
          </p>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            ...
            {DATA_CATEGORIES.map((cat) => {
              const count = cases.filter((c) => c.data_category === cat).length;
              ...
              return (
                ...
                >
                  {DATA_CATEGORY_ICONS[cat]} {DATA_CATEGORY_LABELS[cat]} ({count})
                </button>
              );
            })}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            {filteredCases.map((c) => (
              <ExperimentCaseCard key={c.id} experimentCase={c} />
            ))}
          </div>

          {filteredCases.length === 0 && (
            <div className="text-center py-16">
              <p className="text-gray-400">このカテゴリの事例はまだありません。</p>
            </div>
          )}
        </div>
      )}
        ...
        <div>
          ...
          <div className="md:hidden mb-4 flex items-center justify-between">
            ...
            >
              ...
              フィルタ
              {hasActiveFilters && (
                <span className="inline-flex items-center justify-center w-5 h-5 bg-blue-600 text-white text-xs rounded-full">!</span>
              )}
            </button>
          </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="hidden md:flex items-center justify-between mb-4">
                <p className="text-sm text-gray-500">
                  <span className="font-semibold text-gray-700">{filtered.length}</span> 件のアルゴリズム
                  {hasActiveFilters && (
