...
## Issue #32: IoTセンサー監視データ（Daily Weather 2020, 時系列）

**データ概要**: 13,664行 × 11列、122拠点の日次気象データ、PAR 64ep で10シーケンス生成

### 評価結果

| 指標 | 値 |
|------|-----|
| Quality Score | 0.6444 |
| DCR Mean | 0.5599 |
| Privacy Risk | low |

### 時系列特性

| 列 | 実データ自己相関(lag1) | 合成自己相関(lag1) |
|----|---------------------|-------------------|
| temperatureHigh | 0.9346 | 0.0739 |
| temperatureLow | 0.9467 | 0.1783 |
| humidity | 0.7654 | 0.0738 |
| precipProbability | 0.4706 | 0.0244 |
| lat/long | 0.997 | 0.09-0.14 |

### 知見

1. **自己相関の再現が株価より悪い**: 気象データの自己相関は 0.47〜0.997 だが、合成は 0.02〜0.18 に大幅低下。株価（合成 0.32〜0.76）より悪い。epoch 数が半分（64 vs 128）であることと、変数の多様性（気温・湿度・気圧・降水）が影響している可能性。

2. **lat/long の自己相関問題**: 位置情報（緯度・経度）は拠点内では定数のはずだが、PAR は時系列として学習するため、合成データでは拠点内でも変動してしまう（自己相関 0.09-0.14）。**位置情報のような固定属性は時系列カラムから除外すべき**。

3. **Quality Score は株価より高い (0.64 vs 0.50)**: Column Shapes (75.8%) が比較的高い。気象データの各変数の分布形状は再現できているが、時間方向の依存関係は捉えられていない。SDMetrics が時系列構造を評価しないことの限界が明確。

4. **SDV の download_demo データセット名**: `daily_weather` ではなく `daily_weather_2020` が正しい名前。データセット名は `get_available_demos('sequential')` で確認すること。

### 全6事例の横断的知見

| データタイプ | Quality Score 範囲 | 主な課題 |
|------------|-------------------|---------|
| 単一表（大） | 0.88〜0.90 | TSTR のターゲット列選択 |
| 単一表（小） | 0.59 | 統計的信頼性の限界 |
| 複数表 | 0.52〜0.65 | 列間相関・テーブル間トレンドの再現 |
| 時系列 | 0.50〜0.64 | 自己相関の再現、固定属性の扱い |

- **データ量が多いほど Quality Score が高い**傾向
- **FK 整合性は SDV HMA で常に完全維持**
- **プライバシーリスクは全事例で low**（CTGAN 50ep の Insurance を除く）
- **時系列の自己相関は PAR の弱点** — epoch 数を増やしても根本的改善は限定的
